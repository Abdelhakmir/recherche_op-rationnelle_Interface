...
def potentiel_metra():
    ...
    mpm_window.title("Méthode des Potentiels Métra (MPM)")
    ...
    class MPMApp:
        def __init__(self, root):
            ...
            self.num_tasks_label = ttk.Label(root, text="Entrez le nombre de tâches :")
            self.num_tasks_label.pack()

            self.num_tasks_entry = ttk.Entry(root)
            self.num_tasks_entry.pack()

            self.generate_button = ttk.Button(root, text="Générer le tableau", command=self.generate_table, bootstyle=SUCCESS)
        ...
        def display_table(self):
            ...
            columns = ("ID", "Date au plus tôt (ES)", "Date au plus tard (LS)", "Marge Totale (Slack)", "Dépendances")
        ...
        def apply_mpm(self):
            ...
            for i, task in enumerate(self.tasks):
                ...
                dependencies = ", ".join(task["predecessors"])
                self.treeview.set(self.treeview.get_children()[i], column="Date au plus tôt (ES)", value=es_val)
                self.treeview.set(self.treeview.get_children()[i], column="Date au plus tard (LS)", value=ls_val)
                self.treeview.set(self.treeview.get_children()[i], column="Marge Totale (Slack)", value=slack_val)
                self.treeview.set(self.treeview.get_children()[i], column="Dépendances", value=dependencies)
        ...
        def visualize_graph(self, G, critical_path):
            ...
            plt.title("Diagramme MPM avec chemin critique en surbrillance")
            ...
            self.critical_path_label.config(text=f"Chemin critique : {critical_path_str}")
...
def ford_fulkerson():
    def open_graph_window():
        ...
        num_nodes = simpledialog.askinteger("Entrée", "Combien de nœuds souhaitez-vous ?", minvalue=1)
        ...
        graph_window.title("Graphe Aléatoire")
        ...
        ax.set_title("Graphe Aléatoire avec Capacités")
        ...
        # Add an Apply button
        apply_button = ttk.Button(graph_window, text="Appliquer Ford-Fulkerson", command=lambda: apply_ford_fulkerson(graph, graph_window, pos))
        apply_button.pack(pady=10)

    def apply_ford_fulkerson(graph, window, pos):
        # Ask user for source and sink nodes
        source = simpledialog.askinteger("Entrée", "Entrez le nœud source :")
        sink = simpledialog.askinteger("Entrée", "Entrez le nœud puits :")
        ...
        if source is not None and sink is not None and source in graph.nodes and sink in graph.nodes:
            ...
            nx.draw(graph, pos, with_labels=True, node_color="lightblue", edge_color="gray", node_size=500, font_size=10, ax=ax)
            nx.draw_networkx_edge_labels(graph, pos, edge_labels=capacities)
            ax.set_title("Graphe avec Capacités Mises à Jour après Ford-Fulkerson")
            ...
            result_label = ttk.Label(result_frame, text=f"Flux Maximum : {flow_value}", font=("Helvetica", 14), foreground="blue")
            result_label.pack(pady=10)

            # Display flow details
            flow_details = "\n".join([f"{u} -> {v}: {flow}" for u in flow_dict for v, flow in flow_dict[u].items()])
            flow_text = f"Détails du Flux :\n{flow_details}"
            flow_label = ttk.Label(result_frame, text=flow_text, font=("Helvetica", 12), justify="left")
            flow_label.pack(pady=5, anchor="w")
        else:
            error_label = ttk.Label(window, text="Nœud source ou puits invalide. Veuillez réessayer.", font=("Helvetica", 12), foreground="red")
...
def bellman_ford():
    ...
    def open_bellman_ford_graph_window():
        ...
        # Create a new window
        graph_window = Toplevel(root)
        graph_window.title("Graphe Orienté Aléatoire")
        ...
        ax.set_title("Graphe Orienté Aléatoire avec Poids")
        ...
        # Add an Apply button
        apply_button = ttk.Button(graph_window, text="Appliquer Bellman-Ford", command=lambda: apply_bellman_ford(graph, graph_window))
        apply_button.pack(pady=10)

    def apply_bellman_ford(graph, window):
        start_node = simpledialog.askinteger("Entrée", "Entrez le nœud de départ :")
        ...
        if start_node is not None and start_node in graph.nodes:
            ...
            headers = ["Nœud", "Distance (KM)", "Chemin"]
...
def transportation_problem():
    ...
    transport_window.title("Problème de Transport")
    ...
    class TransportationApp:
        def __init__(self, root):
            ...
            self.north_west_button = ttk.Button(
                button_frame, 
                text="Méthode du Coin Nord-Ouest", 
                bootstyle=SUCCESS,
        ...
        def display_table(self, data, supply, demand):
            ...
            for i, row in enumerate(data):
                row_label = ttk.Label(self.table_frame, text=f"Usine {i + 1}", font=("Helvetica", 12), borderwidth=3, relief="groove")
        ...
        def north_west_corner_method(self):
            ...
            self.optimal_solution_label.config(text=f"Méthode du Coin Nord-Ouest Appliquée\nCoût Total : {total_cost:.2f}")
        ...
        def least_cost_method(self):
            ...
            self.display_table(allocation, self.supply, self.demand)
            self.optimal_solution_label.config(text=f"Méthode du Coût Minimal Appliquée\nCoût Total : {total_cost:.2f}")
            self.stepping_stone_button.pack(side="center", padx=5)
        ...
        def stepping_stone_method(self):
            ...
            if hasattr(self, 'north_west_total') and hasattr(self, 'least_cost_total'):
                ...
                self.optimal_solution_label.config(text="Veuillez d'abord exécuter la méthode du Coin Nord-Ouest ou du Coût Minimal !")
                return
            ...
            if iteration < max_iterations:
                self.optimal_solution_label.config(
                    text=f"Méthode Stepping Stone : Solution Optimale Trouvée\n" +
                         f"Coût Final : {final_cost:.2f}"
                )
            else:
                self.optimal_solution_label.config(
                    text=f"Méthode Stepping Stone : Nombre maximal d'itérations atteint\n" +
                         f"Coût Actuel : {final_cost:.2f}"
                )
...
def dijkstra():
    ...
    def open_dijkstra_graph_window():
        ...
        nx.draw(graph, pos, with_labels=True, node_color="lightblue", edge_color="gray", node_size=500, font_size=10, ax=ax)
        nx.draw_networkx_edge_labels(graph, pos, edge_labels=weights)
        ax.set_title("Graphe Aléatoire avec Poids")
        ...
        # Add an Apply button
        apply_button = ttk.Button(graph_window, text="Appliquer Dijkstra", command=lambda: apply_dijkstra(graph, graph_window))
        apply_button.pack(pady=10)
...
def kruskal():
    ...
    # Create a new window
    graph_window = Toplevel(root)
    graph_window.title("Algorithme de Kruskal - Arbre Couvrant Minimal")
...
def apply_kruskal(graph, window):
    ...
    result_label = ttk.Label(
        window,
        text=f"Poids total de l'ACM : {total_weight} $",
        font=("Arial", 12),
        foreground="blue",
    )
    result_label.pack(pady=10)
...
def main_window():
    ...
    root.title("Abdelhak Mirbah G9 4IIR")
...
def welsh_powell():
    ...
    # Ask user for the number of nodes (sommets)
    num_nodes = simpledialog.askinteger("Entrée", "Combien de sommets souhaitez-vous ?", minvalue=1)
    if num_nodes is None:
        return
    ...
    # Create a new window
    graph_window = Toplevel(root)
    graph_window.title("Graphe Coloré avec Welsh-Powell")
...
def apply_welsh_powell(graph, frame, num_nodes):
    ...
    ax.set_title("Graphe Coloré avec l'Algorithme de Welsh-Powell", pad=20, fontsize=14, fontweight='bold')
    ...
    result_label = ttk.Label(
        result_frame,
        text=f"Plage du Nombre Chromatique : {x1} ≤ X(G) ≤ {x2}",
    ...
    info_label = ttk.Label(
        result_frame,
        text=f"Nombre de Nœuds : {num_nodes} | Couleurs Utilisées : {num_colors_used}",
        font=("Helvetica", 10),
...
def main_window():
    global root  # Make root globally accessible
    root = ttk.Window(themename="flatly")
    root.title("Mirbah Abdelhak 4IIR9")
    root.state("zoomed")  # Set full screen mode
    
    # Set an icon for the title
    try:
        root.iconbitmap("Emsi_logo.ico")
...
intro_window.title("Bienvenue")
intro_window.geometry("1080x1080")
...
try:
    script_dir = os.path.dirname(os.path.abspath(__file__))
    img_path = os.path.join(script_dir, "logo-1.png")
    img = Image.open(img_path)
    # Create rounded corners
    img = img.resize((900, 420))
    mask = Image.new('L', img.size, 0)
    draw = ImageDraw.Draw(mask)
    draw.rounded_rectangle([(0, 0), img.size], radius=0, fill=255)
    output = Image.new('RGBA', img.size, (0, 0, 0, 0))
    output.paste(img, mask=mask)
...
# Create a frame with a custom style and border
text_frame = ttk.Frame(top_frame, style='Custom.TFrame', padding=15)
...
style.configure(
    'Custom.TFrame',
    background='#34495e',  # Subtle light blue background
    borderwidth=2,
    relief='solid'  # Add a solid border around the frame
)
...
made_by_label = ttk.Label(
    ...
    foreground="#34495e",  # Deep grey color
    background="#eaf2f8"
)
...
name_label = ttk.Label(
    text_frame,
    text="Abdelhak Mirbah",
    font=(custom_font, 14, "bold"),
    foreground="#0c9647",  # Bold orange for emphasis
    background="#eaf2f8"
)
